...
void append_page_footer(){ // Saves repeating many lines of code for HTML page footers
  ...
  webpage += F("</ul>");
  webpage += "<footer>Copy@right XuankyAutomation -  Date 10 May 2021 -  Email:phamxuanky82@gmail.com</footer>";
  webpage += F("</body></html>");
}
...
void DownloadFile(String filename){
  if (SPIFFS_present) { 
        File download = Filesystem.open("/"+filename,  "r");
    if (download) {
...
void SPIFFS_dir(){
  ...
  if (SPIFFS_present) { 
    ...
    webpage += F("<table align='center'>");
    webpage += F("<tr><th>Name/Type</th><th style='width:40%'>File Size</th> <th>Download</th><th>Delete</th></tr>");
    while (dir.next()) {
      //Serial.print(dir.fileName());
      webpage += "<tr><td>"+String(dir.fileName())+"</td>";
      str  = dir.fileName();
      //str = substring(1);// (from)or (from,to)
      if(dir.fileSize()) {
        File f = dir.openFile("r");
        //Serial.println(f.size());
        int bytes = f.size();
        String fsize = file_size(bytes);
        webpage += "<td>"+fsize+"</td>";
        webpage += "<td> <a href=/download?download="+String(dir.fileName())+">Download</a> </td>";
        webpage += "<td> <a href=/delete?delete="+String(dir.fileName())+">Delete</a> </td></tr>";  
...
void SPIFFS_file_stream(String filename) { 
  if (SPIFFS_present) { 
      ...
      File dataFile = Filesystem.open("/"+filename,  "r");
      if (dataFile) { 
  ...
  if (SPIFFS_present) { 
     ...
     if (dataFile)
    {
      if (Filesystem.remove(filename)){ 
        webpage += "<h3>File "+filename.substring(2)+ " has been erased</h3>"; 
