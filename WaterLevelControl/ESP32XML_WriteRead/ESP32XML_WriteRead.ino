...
void AnalogPorts(XMLWriter* xw, const char* name)
{
  ...
  xw->writeNode("Analog0", itoa(analogRead(A13), buffer, 10));
  xw->writeNode("Analog1", (uint16_t) analogRead(A12));
  xw->writeNode("Analog2", (double) (5.0*analogRead(A14))/1023);  // default nr decimals = 2
...
void DigitalPorts(XMLWriter* xw)
{
  ...
  xw->tagOpen("Digital");
  xw->writeNode("D27", itoa(digitalRead(1), buffer, 10));
  xw->writeNode("D35", (uint8_t)digitalRead(35));
  xw->tagClose();
}

bool writexml(){  
  uint32_t start = micros();
  File DataFile = Filesystem.open("/data.xml", "w");
  if (!DataFile) {
    Serial.println("Failed to write Setdata.xml file");
    return false;
  }
  ...
  DataFile.close();
  Serial.println("Write Setting Data xml Sussesful");    
  return true;
}
